...
pattern_lo_truoc_thue = re.compile(
    r"Lỗ kế toán trước thuế[\s:]*([\d\.,\-]+)", flags=re.IGNORECASE
)
pattern_lai_sau_thue = re.compile(lai_sau_thue_pt2, flags=re.IGNORECASE)
pattern_lo_sau_thue = re.compile(
    r"Lỗ kế toán sau thuế[\s:]*([\d\.,\-]+)", flags=re.IGNORECASE
)
...
def extract_financial_data(pdf_file):
    ...
    for i, page in enumerate(pages):
        ...
        data.append(
            {
                "TenCongTy": company_name if company_name else None,
                "NgayBaoCao": date_match[0] if date_match else None,
                ...
                "LaiSauThue": lai_sau_thue[0] if lai_sau_thue else None,
